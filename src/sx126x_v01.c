...
{
    digitalWrite(ANT_SWITCH_POWER_Pin, HIGH);
}
...
void SX126xWriteBuffer( uint8_t offset, uint8_t *buffer, uint8_t size )
{
    uint8_t buf[257];
    memset(buf, '\0', 257);
    buf[0]=RADIO_WRITE_BUFFER; ////就是WriteBuffer() 这个命令
    buf[1]=offset;
    SX126xCheckDeviceReady( );
    digitalWrite(SPI_CS_Pin, LOW);
    memcpy(buf+2,buffer,size);
	wiringPiSPIDataRW(0,buf,size+2);                                                                                                                                                        
    digitalWrite(SPI_CS_Pin, HIGH);
    SX126xWaitOnBusy( );
}
...
void SX126xReadBuffer( uint8_t offset, uint8_t *buffer, uint8_t size )
{
    ...
    buf[0]=RADIO_READ_BUFFER;  ////就是ReadBuffer() 这个命令
    buf[1]=offset;
    buf[2]=0;
    SX126xCheckDeviceReady( );
    digitalWrite(SPI_CS_Pin, LOW);
    wiringPiSPIDataRW(0,buf,size+3);
...
void SX126xWriteCommand( RadioCommands_t command, uint8_t *buffer, uint16_t size )
{
  ...
  wiringPiSPIDataRW(0,buf,size+1);
  digitalWrite(SPI_CS_Pin, HIGH);
...
void SX126xWriteRegisters( uint16_t address, uint8_t *buffer, uint16_t size )
{
    ...
    uint8_t buf[258];
    memset(buf, '\0', 258);
    addrh = (address&0xFF00)>>8; //高8位
    addrl = (address&0x00FF);    //低8位
...
void SX126xReadRegisters( uint16_t address, uint8_t *buffer, uint16_t size )
{
	...
	SX126xCheckDeviceReady( );
    digitalWrite(SPI_CS_Pin, LOW);
    wiringPiSPIDataRW(0,buf,size+4);
	memcpy(buffer,buf+4,size);
    digitalWrite(SPI_CS_Pin, HIGH);
    SX126xWaitOnBusy( ); 
}
